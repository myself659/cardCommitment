...
void cardHash_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void cardHash_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
ctx->componentMemory[coffset].templateName = "cardHash";
ctx->componentMemory[coffset].signalStart = soffset;
...
cardHash_2_create(1,0,ctx,"main",0);
cardHash_2_run(0,ctx);
}
